...
const taskCount = prd.items.filter((i) => !i.passes).length;
const scale =
...
- coordinator-state.json (minimal fields)
- current-task.json
...
function detectScale(prd) {
  ...
  const depCount = prd.items.reduce((sum, i) => sum + i.dependencies.length, 0);
...
- [ ] Document scale in coordinator-state.json
...
- [agents/pm/skills/skill-improvement.md](skill-improvement.md) — Skill updates

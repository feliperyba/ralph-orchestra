...
Configuration validation schemas are provided in `.claude/schemas/`:

### agent-config.schema.json

Defines the structure for agent configurations generated by PRD Starter:
...
### prd-starter-state.schema.json

Defines the state structure for the PRD Starter wizard:

- 8-phase workflow state tracking
- Phase completion status
- Research findings storage
- Resume capability support

## PRD Starter State File

The PRD Starter wizard maintains its state in `.claude/session/prd-starter-state.json`:
...
```json
{
  ...
  "phases": {
    "project_identification": { "completed": true, ... },
    "agent_configuration": { "completed": false, ... },
    ...
  }
}
```

This allows resuming the wizard from any phase if interrupted. See [PRD Starter Guide](../quick-start/prd-starter.md) for details.

...
### Client-Side State Access Prevention

**CRITICAL**: Never expose sensitive game state to all clients. Use `@private()` decorator for sensitive fields.

```typescript
import { Schema, type, private } from '@colyseus/schema';

class PlayerState extends Schema {
  @type('string') displayName: string = '';  // Public

  // Sensitive - clients should NOT see this
  @private() internalId: string = '';

  @private() reportCount: number = 0;  // Anti-abuse tracking

  @private() trustScore: number = 100;  // Hidden reputation
}
```

### State Mutation Protection

Clients should only be able to modify state through validated server messages, never directly.

```typescript
// WRONG: Client can modify state directly
onMessage(client: Client, data: any) {
  const player = this.state.players.get(client.sessionId);
  player.score = data.score;  // Client sets score directly!
}

// RIGHT: Server validates and controls all changes
onMessage(client: Client, data: any) {
  const player = this.state.players.get(client.sessionId);

  // Validate score change is legitimate
  const scoreDelta = data.scoreDelta || 0;
  if (scoreDelta > 100) {
    console.warn(`Suspicious score change from ${client.sessionId}`);
    return;
  }

  // Server controls the actual state change
...
Large schemas can cause performance issues and bandwidth problems.

```typescript
// ❌ TOO LARGE - ArraySchema with thousands of items
class MatchState extends Schema {
  @type([Particle]) particles = new ArraySchema<Particle>();
  // Can grow to 10,000+ items = 1MB+ per sync!
}

// ✅ Bounded - Limit array size
class MatchState extends Schema {
  @type([Particle]) particles = new ArraySchema<Particle>();
...
```typescript
// Don't let clients flood state with garbage data
onMessage(client: Client, data: any) {
  if (data.type === 'place_marker') {
    const playerMarkers = this.state.markers.get(client.sessionId) || [];

    // Limit markers per player
    if (playerMarkers.length >= 50) {
      return; // Reject - too many markers
    }

    // Validate marker position
    if (!isValidPosition(data.x, data.z)) {
      return;
    }

    playerMarkers.push({ x: data.x, z: data.z });
    this.state.markers.set(client.sessionId, playerMarkers);
  }
}
```
...
```typescript
// Don't sync high-frequency changes unnecessarily
class PlayerState extends Schema {
  @type('uint16') score: number = 0;
  @type('uint8') health: number = 100;

  // For rapidly changing values, consider throttling
  private lastSyncTime = 0;
  private pendingPosition = { x: 0, y: 0, z: 0 };

  setPosition(x: number, y: number, z: number) {
    this.pendingPosition = { x, y, z };

    // Only sync every 50ms max (20fps for position updates)
    const now = Date.now();

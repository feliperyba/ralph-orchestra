---
name: coverage-tracking-pattern
description: Grid-based surface coverage tracking for territorial game mechanics
category: gameplay
depends-on: [r3f-fundamentals, typescript-patterns]
---

# Coverage Tracking Pattern Skill
...
```tsx
// Grid-based coverage tracking (from PaintDecalManager)
...
```
PaintDecalManager (spawns decals)
         ↓
    recordPaint() (called on each decal spawn)
         ↓
    SurfacePaintData (aggregates by grid cell)
         ↓
    calculateCoverage() (returns percentages)
         ↓
    Game Score (team territory control)
```
...
```tsx
// From PaintDecalManager - overlap handling
...
```tsx
// Polling (simpler, from PaintDecalManager)
useEffect(() => {
  const interval = setInterval(() => {
    const coverage = calculateCoverage(allSurfaces);
    onCoverageChange?.(coverage);
  }, 1000); // Every second

  return () => clearInterval(interval);
}, [allSurfaces, onCoverageChange]);

// Event-driven (more efficient, for v2)
const coverageListeners = new Set<(result: CoverageResult) => void>();

function notifyCoverageChange(result: CoverageResult) {
  coverageListeners.forEach(cb => cb(result));
}

// Only notify when significant change (>1%)
let lastNotified: CoverageResult | null = null;

function checkAndNotify(current: CoverageResult) {
  if (!lastNotified) {
    notifyCoverageChange(current);
    lastNotified = current;
    return;
  }

  const significantChange = Object.entries(current.coverage).some(
    ([team, pct]) => Math.abs(pct - (lastNotified.coverage[team] || 0)) > 1
  );
...
```tsx
// usePaintCoverage hook (from PaintDecalManager)
function usePaintCoverage(
  updateInterval: number = 1000
): CoverageResult | null {
  const [coverage, setCoverage] = useState<CoverageResult | null>(null);
  const manager = usePaintDecalManager();

  useEffect(() => {
    if (!manager) return;

    const interval = setInterval(() => {
      const result = manager.calculateCoverage();
      setCoverage(result);
    }, updateInterval);
...
// Usage in HUD
function HUD() {
  const coverage = usePaintCoverage();
  ...
  return (
    <div className="hud-scores">
      <div className="team-orange">
        {coverage.coverage.orange?.toFixed(1)}%
      </div>
      <div className="team-blue">
        {coverage.coverage.blue?.toFixed(1)}%
      </div>
    </div>
  );
}

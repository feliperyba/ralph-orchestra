...
> **Note**: For FBX models, see `fbx-loading-guide.md`. For game development patterns, see `game-patterns.md`. Both are accessible from the main SKILL.md reference table.

---

## Loading 3D Models (FBX)
...
<script type="module">
    ...
    loader.load(
        'path/to/model.fbx',
        (fbx) => {
            // FBXLoader returns the object directly (not a wrapper like glTF)
            fbx.traverse((child) => {
                if (child.isMesh) {
...
**Note:** For React Three Fiber, use the `useFBX` hook from `@react-three/drei`:
```javascript
import { useFBX } from '@react-three/drei';

function Model() {
    const fbx = useFBX('/path/to/model.fbx');

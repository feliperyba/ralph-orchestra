...
### Loading Environment
- 3D models (FBX, glTF) must be served over HTTP (avoid `file://`), or loaders may fail silently / behave differently.
...
4) Print animation clip names:
   - `console.log(model.animations.map(a => a.name))`  // Works for both FBX and glTF

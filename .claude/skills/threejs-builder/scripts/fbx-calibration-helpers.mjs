// FBX Calibration Helpers (Three.js)
...
//   attachFbxCalibrationHelpers({ scene, root: modelRoot, label: 'Hero' });
//
// Notes:
...
function ensureLayer(root) {
  ...
  layer.name = '__fbx_calibration_helpers__';
  root.add(layer);
  return layer;
}

function removeExisting(root) {
  const existing = root.getObjectByName('__fbx_calibration_helpers__');
...
export function attachFbxCalibrationHelpers({
  scene = null,
  root,
  label = 'model',
  ...
  if (!root) throw new Error('attachFbxCalibrationHelpers: missing { root }');
  ...
  if (showGrid && scene) {
    ...
    grid.name = '__fbx_calibration_grid__';
    scene.add(grid);
  }
  ...
  if (log) {
    ...
    console.log(`[fbx-calibrate] ${label}`);

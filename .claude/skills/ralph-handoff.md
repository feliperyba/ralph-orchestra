...
1. **Save all state to files**:
   - Update `coordinator-state.json` with current status
   - Update `current-task.json` with task progress
...
Read coordinator-state.json and current-task.json for full state.
```

**Your first action should be:**

1. Acknowledge the handoff: "Received handoff from PM for task feat-001"
2. Read state files to get full context

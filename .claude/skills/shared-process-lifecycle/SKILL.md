...
if ($existing) {
    ...
    # Start new process
    $process = Start-ManagedProcess -Name "dev-server" -Port 3000 -Command "npm run dev:all:sh"
}
```
...
# 2. Start if needed
if (-not $server) {
    $server = Start-ManagedProcess -Name "dev-server" -Port 3000 -Command "npm run dev:all:sh"
...
# ... update prd.json.agents.{agent}.status to "idle" ...
```

...
**CRITICAL**: Server must perform ALL hit detection. Client optimism is visual only.

```typescript
// ❌ WRONG - Client sends hit confirmation
onMessage(client: Client, data: any) {
  // Client says they hit player X - TRUSTING CLIENT = CHEATING
  if (data.type === 'hit_confirm') {
    this.damagePlayer(data.targetId, data.damage);
  }
}

// ✅ RIGHT - Server validates all hits
update(dt: number) {
  for (const proj of this.projectiles) {
    // Server performs raycast
    const hit = this.raycast(proj.origin, proj.direction);

    if (hit) {
      // Only server determines hits
      this.applyDamage(hit.entity, proj.damage);
      this.broadcastHitEvent(proj, hit);
    }
  }
}
```
...
Prevent unlimited ammo exploits by tracking server-side.

```typescript
class AmmoValidator {
  // Validate player has ammo before shooting
  canShoot(player: PlayerState): boolean {
    ...
    // Check refill cooldown (prevent instant refill exploits)
    const now = Date.now();
    if (player.ink < 100 && now - player.lastRefillTime < 5000) {
      // Still refilling - use current value
      return player.ink > 0;
    }
  ...
  consumeAmmo(player: PlayerState, amount: number) {
    if (player.ink >= amount) {
      player.ink -= amount;
      return true;
    }
    return false;
  ...
  // Validate refill is legitimate (not hacked)
  canRefill(player: PlayerState): boolean {
    const now = Date.now();
    const timeSinceRefill = now - player.lastRefillTime;

    // Min 5 seconds between refills
    if (timeSinceRefill < 5000) {
      console.warn(`Refill spam detected from ${player.clientId}`);
      return false;
    }
...
Prevent rapid-fire exploits that bypass intended fire rate limits.
...
processFireMessage(client: Client, data: FireMessage) {
  ...
  // ✅ RIGHT - Strict fire rate enforcement
  const now = Date.now();
  const cooldownMs = 100; // 600 RPM = 100ms between shots

  if (now - player.lastShotTime < cooldownMs) {
    // Too fast - reject shot
    this.sendReject(client, data.sequence, 'cooldown');
    return;
  }

  // Check for packet manipulation (very old sequence numbers)
  if (data.sequence < player.lastProcessedSequence) {
    console.warn(`Old shot sequence from ${client.sessionId}`);
    return;
  }

  // Valid shot - create projectile server-side
  this.createProjectile(player, data);
  player.lastShotTime = now;
}
```

### Aim Direction Validation

Validate aim direction to prevent shooting through walls or invalid targeting.

```typescript
validateAimDirection(player: PlayerState, direction: Vector3): boolean {
  // 1. Check direction is normalized (unit vector)
  ...
  if (length < 0.9 || length > 1.1) {
    console.warn('Invalid aim direction (not normalized)');
    return false;
  }

  // 2. Check direction is within valid range (no shooting backwards)
  // Assuming y is up, player can aim -80 to +80 degrees vertically
  const forwardDot = direction.z; // Simplified
  if (forwardDot < 0.17) { // ~80 degrees
    console.warn('Aim direction too far backwards');
    return false;
  }

  // 3. Optional: Check for impossible snap turns
  // Player can't instantly rotate 180 degrees
  if (this.isImpossibleTurn(player, direction)) {
    console.warn('Impossible turn detected');
    return false;
  }
...
### Optimistic Effect Limits

Limit pending optimistic effects to prevent resource exhaustion attacks.

```typescript
class OptimisticDecalManager {
  private pendingDecals = new Map<string, PendingDecal>();
  private readonly maxPending = 50;

  canSpawnDecal(): boolean {
    // Prevent client from spawning unlimited decals
    if (this.pendingDecals.size >= this.maxPending) {
      console.warn('Too many pending decals - possible exploit');
      return false;
    }
    return true;
  }

  spawnDecal(id: string, sequence: number, position: Vector3) {
    if (!this.canSpawnDecal()) {
      return;
    }

    this.pendingDecals.set(id, {
      id,
      sequence,
      optimistic: true,
      confirmed: false,
  ...
  // Force cleanup if server is unresponsive
  cleanupOldDecals() {
    const now = Date.now();
    const maxAge = 3000; // 3 seconds max

    for (const [id, decal] of this.pendingDecals) {
      if (decal.optimistic && now - decal.createdAt > maxAge) {
        this.removeDecal(id);
      }
    }
  }
